...
class Px2vw {
                ...
                for (let index = 0; index < declarations.length; index++) {
                    ...
                    if (declaration.type === 'declaration' && config.regExp.test(declaration.value)) {
                        ...
                        if (nextNode && nextNode.type === 'comment') {
                            ...
                            } else if (nextNode.comment.trim() === config.forceComment) {
                                if (declaration.value === `0${config.forceComment}`) {
                                    declaration.value = '0'
                                } else {
                                    declaration.value = self.calculateValue('vw', declaration.value)
                                }
                                declarations.splice(nextIndex, 1)
                            } else {
                                declaration.value = self.calculateValue('vw', declaration.value)
    ...
    calculateValue (type, value) {
        ...
        return value.replace(regExpGlobal, function ($0, $1) {
            return getValue($1 * multipleNumber / config.datum)
        })
    }
}
